...
int main(int argc, char *argv[]) {
  ...
  struct mosquitto *mosq = NULL;
	//int data[4];
	org_eclipse_tahu_protobuf_Payload_DataSet_Row row_data[240];
	org_eclipse_tahu_protobuf_Payload_DataSet_DataSetValue elements[240];
	uint32_t datatypes[1];
	const char *column_keys[1] = {"data"};
	int i;
	int qty = 240;
	int type = 0;
  ...
  //metrics.value.dataset_value = org_eclipse_tahu_protobuf_Payload_DataSet_init_zero;
	...
	datatypes[0] = DATA_SET_DATA_TYPE_INT32;
	for(i=0; i<qty; i++){
		row_data[i].elements_count = 1;
		row_data[i].elements = &elements[i];
		row_data[i].extensions = NULL;
	...
	metrics.value.dataset_value.num_of_columns = 1;
	metrics.value.dataset_value.columns_count = 1;
	metrics.value.dataset_value.columns = (char**)column_keys;
	metrics.value.dataset_value.types_count = 1;
	metrics.value.dataset_value.types = datatypes;
	metrics.value.dataset_value.rows_count = qty;
  ...
  mosquitto_publish(mosq, NULL, "spBv1.0/dev/DCMD/Teensy1/data", message_length, binary_buffer, 0, false);

	int total = sizeof(payload) + sizeof(metrics) + sizeof(row_data) + sizeof(elements);
	total += sizeof(column_keys) + sizeof(datatypes);
	printf("total size: %i\n\n", total);
	// Close and cleanup
