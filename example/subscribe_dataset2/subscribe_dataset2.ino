...
This program expects a 1-row by 240-column data set.
Columns are 32-bit signed integer

*** Important Notes ***
- payload must be free'd when we're done with it, and before attempting to call
  decode agin. Failure to do so will result in memory leaks and undefined behavior
...
const char* MQTT_TOPIC = "spBv1.0/dev/DCMD/Teensy1/data2";
...
//org_eclipse_tahu_protobuf_Payload_DataSet_Row rows[1];
  ...
  if(!res){
    ...
    if(String(topic).compareTo(MQTT_TOPIC) == 0){
      if(spark.payload.metrics_count == 1){
        ...
        t0 = millis();
        for(i=0;i<nCol;i++){
          data[i] = (int)spark.payload.metrics[0].value.dataset_value.rows[0].elements[i].value.int_value;
        }
        dt = millis() - t0;
...
void setup() {
  ...
  // ~~~~~ PubSubClient ~~~~~~~
  mmqtClient.setBufferSize(3000); // set the buffer size, default is 256
  // ~~~~ End PubSubClient ~~~~
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          client.println("<br><br>" + mqtt_msg);
          //client.println("<br><br>" + String(rx_as_hex));

          if(nCol > 0){
            client.println(String(1) + "> " + String(data[0]) + "<br>");
            client.println(String(nCol) + "> " + String(data[nCol-1]) + "<br>");
          }
